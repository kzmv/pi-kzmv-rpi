...
- If `{working-dir}/specification/{spec-name}__plan.md` exists:
...
- `{working-dir}/specification/{spec-name}__analysis.md` — acceptance criteria and edge cases
- `{working-dir}/specification/{spec-name}__research.md` — reference links and implementation context
...
- There are no unresolved `[NEEDS CLARIFICATION]` items in `__analysis.md` that block this work. If there are, surface them and wait for the user to resolve them.
...
3. Verify the outcome against the step goal and any relevant acceptance criteria from `__analysis.md`.
...
If executing a phase file and an index (`__plan.md`) exists, update the Phase Breakdown table in the index to mark that phase done. Add a `Status` column to the table if not already present:

```markdown
| Phase | File | Description | Status |
|---|---|---|---|
| 1 | `{spec-name}/plan/01-{phase-name}.md` | [Description] | ✓ Done |
| 2 | `{spec-name}/plan/02-{phase-name}.md` | [Description] | — |
```
...
- [ ] Acceptance criteria from `__analysis.md` are met for this plan or phase
...
4. **Acceptance criteria met:** list which criteria from `__analysis.md` are now satisfied
